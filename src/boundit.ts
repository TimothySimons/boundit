...
export function BoundItCanvas(canvas: HTMLCanvasElement, imgURL: string): Api {
  ...
  img.onload = function () {
    ...
    if (Math.abs(imgAspectRatio - canvasAspectRatio) > 0.01) {
      throw new Error(
        `Canvas size is not proportional to the image size: (${canvas.width}, ${canvas.height}) (${img.naturalWidth}, ${img.naturalHeight})`,
      );
    }
  ...
  return {
    ...
    getStyle(state: BoxState) {
      return boxStyles[state];
    },
    setStyle(state: BoxState, style: Style) {
      boxStyles[state] = style;
    },
    getBoxes() {
      return boxes.map(box => canvasBoxToImageBox(box, img, canvas));
    },
  };
}
...
function redraw(
  ...
  boxes: CanvasBox[],
  boxStyles: Record<BoxState, Style>,
) {
